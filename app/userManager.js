...
const createUser = async(req, res) => {
    const data = req.body
    const sql = 'INSERT INTO user (username, password_hash, name, dob, gender, email, phone, user_type, inserted_at, updated_at) VALUES ?';
    const values = data.map(item => [item.username, sha256(item.password), item.name, item.dob, item.gender, item.email, item.phone, item.user_type, new Date().toISOString().slice(0, 19).replace('T', ' '), new Date().toISOString().slice(0, 19).replace('T', ' ')]);
...
const addAddress = async(req, res) => {
    ...
    values.push([user_id, new Date().toISOString().slice(0, 19).replace('T', ' '), new Date().toISOString().slice(0, 19).replace('T', ' ')])
...
const updateAddress = async(req, res) => {
    ...
    const values = Object.values(newData);
    values.push([new Date().toISOString().slice(0, 19).replace('T', ' '), id])
